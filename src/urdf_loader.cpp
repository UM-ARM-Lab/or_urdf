                     ...
                     std::vector<OpenRAVE::KinBody::JointInfoPtr> &joint_infos)
{
    ...
    BOOST_FOREACH (urdf::LinkSharedPtr link, link_vector) {
   ...
   urdf::LinkConstSharedPtr link_ptr;
    ...
    while (!link_list.empty()) {
        ...
        urdf::InertialSharedPtr inertial = link_ptr->inertial;
        if (inertial) {
        ...
        urdf::JointSharedPtr parent_joint = link_ptr->parent_joint;
        while (parent_joint) {
        ...
        BOOST_FOREACH (const std::shared_ptr<urdf::Collision> collision,
        ...
        urdf::VisualSharedPtr visual = link_ptr->visual;
        if (visual) {
            OpenRAVE::KinBody::GeometryInfoPtr geom_info
                = boost::make_shared<OpenRAVE::KinBody::GeometryInfo>();
            ...
            case urdf::Geometry::MESH: {
                ...
                urdf::MaterialSharedPtr material = visual->material;
                if (material) {
    ...
    std::vector<urdf::JointSharedPtr > ordered_joints;
    BOOST_FOREACH(boost::tie(joint_name, joint_ptr), model.joints_) {
        ordered_joints.push_back(joint_ptr);
    }

    BOOST_FOREACH(urdf::JointSharedPtr joint_ptr, ordered_joints) {
        ...
        urdf::JointMimicSharedPtr mimic = joint_ptr->mimic;
        if (mimic) {
        ...
        urdf::JointLimitsSharedPtr limits = joint_ptr->limits;
        if (limits) {
                           ...
                           std::vector<OpenRAVE::RobotBase::ManipulatorInfoPtr> &manip_infos)
{
    ...
    BOOST_FOREACH (srdf::Model::LinkSpheres const &spheres,
        ...
        typedef urdf::LinkConstSharedPtr LinkConstPtr;
        urdf::LinkConstSharedPtr urdf_link = urdf.getLink(spheres.link_);
        BOOST_ASSERT(urdf_link);
        const urdf::CollisionSharedPtr collision = urdf_link->collision;
